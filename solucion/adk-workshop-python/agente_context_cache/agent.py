...
def after_model_callback(
    ...
    """
    Función que intercepta la respuesta del modelo antes de mostrarla en adk web.
    """
    if llm_response.usage_metadata:
        usage = llm_response.usage_metadata
        # Obtener la cantidad de tokens servidos desde el caché
        cached_tokens = getattr(usage, 'cached_content_token_count', 0)
        total_tokens = usage.total_token_count
        
        # Mostrar en el Log de la terminal (donde corre 'adk web')
        if cached_tokens and cached_tokens > 0:
            print(f"✅ ¡Uso de Caché Detectado! Tokens cacheados: {cached_tokens}")
        else:
            print("❌ Sin uso de Caché de Contexto en esta turno.")
            
        # Inyectar el estado del caché visualmente en el chat de 'adk web'
        cache_status_msg = (
            f"\n\n---\n📊 **Metadatos de Contexto:** "
            f"Tokens totales: `{total_tokens}` | "
            f"Tokens desde Caché: `{cached_tokens}`"
        )
        
        if llm_response.content and llm_response.content.parts:
            # Agregamos un nuevo bloque de texto a la respuesta que verá el usuario
            llm_response.content.parts.append(
                types.Part.from_text(text=cache_status_msg)
            )
